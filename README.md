...
- 🛑 **停止生成**：回复中点击按钮即可停止
- 📂 **多会话管理**：会话列表、切换、删除、**重命名**，历史自动保存（本地）
- 🔍 **消息搜索**：跨会话全文搜索历史消息，点击结果直达会话
- 🔔 **回复完成通知**：切到后台时回复完成弹出系统通知
- 📥 **导出对话**：单个会话或全部会话导出为 Markdown 文件下载
- ✨ **代码高亮**：代码块关键词/字符串/注释自动着色（零依赖）
- 🧹 **数据管理**：一键清空全部会话（保留 API Key）
- 🎨 **主题切换**：深色 / 浅色 / 跟随系统，一键切换
- ⇪ **分享对话**：一键导出对话文本（系统分享 / 复制）
- 📋 **消息操作**：复制任意消息
...
```powershell
node test-mobile.js   # 40 项断言：存储/多会话/重命名/清空/推理/余额
node test-md.js       # 24 项断言：Markdown 渲染/高亮/XSS 安全
